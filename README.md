...
```
###############################
Try creating composite outcome variable with log of counts
```{r}
head(HCS_Passport)
sum(is.na(HCS_Passport))


hist(scale(HCS_Passport$MissedVisits))
mean(HCS_Passport$MissedVisits)
median(HCS_Passport$MissedVisits)
sd(HCS_Passport$MissedVisits)
qqnorm(HCS_Passport$MissedVisits)

## Seems that logging it makes it worse
mean(scale(log(HCS_Passport$MissedVisits)))
median(scale(log(HCS_Passport$MissedVisits)))
shapiro.test(log(HCS_Passport$MissedVisits))


qqnorm(scale(log(HCS_Passport$MissedVisits)))
qqnorm(scale(HCS_Passport$MissedVisits))

## Now look at NumServices
hist(HCS_Passport$NumServices)
hist(log(HCS_Passport$NumServices))
qqnorm(HCS_Passport$NumServices)
qqnorm(log(HCS_Passport$NumServices))
mean(log(HCS_Passport$NumServices))
median(log(HCS_Passport$NumServices))
sd(HCS_Passport$NumServices)
hist(HCS_Passport$MissedVisits)

HCS_Passport$PharmCost = gsub("\\D", "", HCS_Passport$PharmCost)
HCS_Passport$PPTotalPaid = gsub("\\D", "", HCS_Passport$PPTotalPaid)

write.csv(HCS_Passport, "HCS_Passport.csv", row.names = FALSE)
HCS_Passport = read.csv("HCS_Passport.csv", header = TRUE)
HCS_Passport

HCS_PassportOutcome = data.frame(MissedVisits = scale(HCS_Passport$MissedVisits), NumServices = scale(log(HCS_Passport$NumServices)), PharmCost = scale(HCS_Passport$PharmCost), PPTotalPaid = scale(HCS_Passport$PPTotalPaid))

HCS_PassportOutcome = rowMeans(HCS_PassportOutcome)
describe(HCS_PassportOutcome)


HCS_Passport$HCS_PassportOutcome = HCS_PassportOutcome
...
##"manage_health", "manage_mhealth", "similar_goals", "no_concern_side_effects", "has_money_food", "health_friendly_home", "has_transport", "social_activity" , "has_money_for_health", "ed_level_satisfaction"
HCSTotalScore = HCS_Passport[c("ManageHealthProblems", "ManageMentalWellness", "ProvidersSimilarGoals", "NoSideEffectsConcerns", "NutritiousFoodFinancialBarrier", "HealthyHomeEnvironment", "TransportationAccess", "ParticipateSocialActivity", "ManageFinances")]

HCSTotalScore = rowSums(HCSTotalScore)

HCS_ML = data.frame(HCSTotalScore = HCSTotalScore, HCS_PassportOutcome = HCS_PassportOutcome,FollowUpTimePoint = HCS_Passport$FollowUpTimePoint)
HCS_ML
cor.test(HCS_ML$HCSTotalScore, HCS_ML$HCS_PassportOutcome)

describe.factor(HCS_ML$FollowUpTimePoint)
HCS_ML$FollowUpTimePoint = ifelse(HCS_ML$FollowUpTimePoint == "Intake", 0, ifelse(HCS_ML$FollowUpTimePoint == "3 Month", 1, 2))


```
Now put together machine learning
Let us get the training and actual data set
```{r}
inTrain = createDataPartition(y = HCS_ML$HCS_PassportOutcome, p = .50, list = FALSE)
training = HCS_ML[inTrain,]
testing = HCS_ML[-inTrain,] 

```
Now selecting the cross validation
```{r}
fitControl <- trainControl(
  ...
  repeats = 10)
```
Now we need to figure out which method to use.  GBM seems to be good for binary outcomes.  We need one that is good for longituidnal data and continious outcomes
```{r}
set.seed(12345)
gbmFit1 <- train(HCS_PassportOutcome ~ ., data = training, 
                 method = "gbm", 
...
```
Claims data
Think about adding a year variable later
```{r}
head(Claims2017)
...
```
############################################################################################
Merging with HCS data first to get matches then merge with all data later for large data set
############################################################################################
Now start merging with Claims first and moving from there
HCS_Passport
```{r}
head(HCS_Passport)
names(HCS_Passport)[1] =  "AvatarClient_ID"
head(Claims2017_Claims2018)
HCS
dim(Claims2017_Claims2018)

PassportData = merge(Claims2017_Claims2018, HCS_Passport, by  = "AvatarClient_ID", all.y = TRUE)

dim(PassportData)
describe.factor(PassportData$TakePrescribedMeds)
PassportData
```
Now merge these variables
PHARMCLAIMS_2017_2018
InpatClaimsData
InpatAvatarData 
OutpatClaimsData

Ok so only thing that matches is the Info data set.  Need to look into that
```{r}
PassportData = merge(PassportData, Member2017_2018, by  = "AvatarClient_ID", all.x = TRUE)
head(PassportData)

PassportData = merge(PassportData, PHARMCLAIMS_2017_2018, by = "AvatarClient_ID", all.x = TRUE)

PassportData = merge(PassportData, InpatClaimsData, by = "AvatarClient_ID", all.x = TRUE)

PassportData = merge(PassportData, InpatAvatarData, by = "AvatarClient_ID", all.x = TRUE)

PassportData = merge(PassportData, OutpatClaimsData, by = "AvatarClient_ID", all.x = TRUE)

```
##################################################
Now try merging all data sets together
############################################################################################
Now start merging with Claims first and moving from there
HCS_Passport
```{r}
...
PassportData = merge(Claims2017_Claims2018, HCS_Passport, by  = "AvatarClient_ID", all = TRUE)
...
Ok need to aggregate by POS on Claims first then merge
```{r}
PassportData = merge(PassportData, Member2017_2018, by  = "AvatarClient_ID", all = TRUE)
head(PassportData)
